...
# LR model with optimized hyperparameters
# Load the saved parameters from the JSON file
with open(HYPERPARAMETERS_FOLDER + SYSTEM_SEP +'best_hyperparameters_LR.json', 'r') as f:
...
for run in range(n_runs):
    ...
    LR_model = LogisticRegression(**loaded_params)
    LR_model.fit(SX_train, ytrain_np)

    
    # Collect metrics for training, validation, and test sets
    train_metrics = calculate_metrics(SX_train, ytrain_np, LR_model)
    test_metrics = calculate_metrics(SX_test, ytest_np, LR_model)
    val_metrics = calculate_metrics(SXval, yval_np, LR_model)
...
joblib.dump(best_model, MODEL_FOLDER + SYSTEM_SEP + f'best_LR_noki_model_run_{best_run}.joblib')
