...
XBG_model = joblib.load(MODEL_FOLDER + SYSTEM_SEP + 'best_ki_filtered_xgboost_model_run_3.joblib') #Change the run number, if it is different
# Load best LightGBM base model
LightGBM_model = joblib.load(MODEL_FOLDER + SYSTEM_SEP + 'best_ki_filtered_lgbm_model_run_1.joblib') #Change the run number, if it is different
# Load best Random Forest base model
RF_model = joblib.load(MODEL_FOLDER + SYSTEM_SEP + 'best_ki_filtered_rf_model_run_3.joblib') #Change the run number, if it is different
...
for run in range(n_runs):
    ...
    meta_model = lgb.LGBMClassifier(**loaded_params)
...
joblib.dump(best_model, MODEL_FOLDER + SYSTEM_SEP + f'best_ki_filtered_meta_model_run_{best_run}.joblib')
...
plt.title('Top 30 Important Features (GPCR-A17 MAAP (Ki-filtered))', fontsize=16, fontweight='bold')
