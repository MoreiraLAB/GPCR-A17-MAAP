...
# Load datasets
Dataset_final = pd.read_csv(FEATURES_FOLDER + SYSTEM_SEP + 'GPCRA17_ki_filtered_features_Mold2_PT.csv', low_memory=False)  
DNS_dataset = pd.read_csv(FEATURES_FOLDER + SYSTEM_SEP + 'GPCRA17_DNS_ki_filtered_features_Mold2_PT.csv', low_memory=False)
...
X = Dataset_final.drop(columns =['Receptor', 'Smiles', 'Ligand_ID', 'Database', 'Class'], axis=1)
y = Dataset_final['Class']

X_DNS = DNS_dataset.drop(columns =['Receptor', 'Smiles', 'Ligand_ID', 'Database', 'Class'], axis=1)
y_DNS = DNS_dataset['Class']

# Save the processed datasets to CSV files
X.to_csv(SPLITS_FOLDER + SYSTEM_SEP + 'X_GPCRA17_ki_filtered_Mold_PT.csv', index=False)
y.to_csv(SPLITS_FOLDER + SYSTEM_SEP + 'y_GPCRA17_ki_filtered_Mold_PT.csv', index=False)
X_DNS.to_csv(SPLITS_FOLDER + SYSTEM_SEP + 'X_DNS_ki_filtered_Mold_PT.csv', index=False)
y_DNS.to_csv(SPLITS_FOLDER + SYSTEM_SEP + 'y_DNS_ki_filtered_Mold_PT.csv', index=False)
