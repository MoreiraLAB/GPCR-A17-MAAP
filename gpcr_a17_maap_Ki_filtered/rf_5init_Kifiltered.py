...
with open(HYPERPARAMETERS_FOLDER + SYSTEM_SEP + 'best_hyperparameters_rf_ki_filtered.json', 'r') as f:
...
for run in range(n_runs):
    ...
    rf = RandomForestClassifier(**loaded_params)
    rf.fit(SX_train, ytrain_np)
    
  # Collect metrics for training, validation, and test sets
    train_metrics = calculate_metrics(SX_train, ytrain_np, rf)
    test_metrics = calculate_metrics(SX_test, ytest_np, rf)
    val_metrics = calculate_metrics(SXval, yval_np, rf)
...
joblib.dump(best_model, MODEL_FOLDER + SYSTEM_SEP + f'best_ki_filtered_rf_model_run_{best_run}.joblib')
...
plt.title('Top 30 Important Features (Random Forest (Ki-filtered))', fontsize=16, fontweight='bold')
