...
with open(HYPERPARAMETERS_FOLDER + SYSTEM_SEP +'best_hyperparameters_xgb_ki_filtered.json', 'r') as f:
...
for run in range(n_runs):
    ...
    xgb_model = xgb.XGBClassifier(**loaded_params)
    xgb_model.fit(SX_train, ytrain_np)
    
  # Collect metrics for training, validation, and test sets
    train_metrics = calculate_metrics(SX_train, ytrain_np, xgb_model)
    test_metrics = calculate_metrics(SX_test, ytest_np, xgb_model)
    val_metrics = calculate_metrics(SXval, yval_np, xgb_model)
...
joblib.dump(best_model, MODEL_FOLDER + SYSTEM_SEP + f'best_ki_filtered_xgboost_model_run_{best_run}.joblib')
...
plt.title('Top 30 Important Features (XGBoost (Ki-filtered))', fontsize=16, fontweight='bold')
