...
with open(HYPERPARAMETERS_FOLDER + SYSTEM_SEP +'best_hyperparameters_KNN.json', 'r') as f:
...
for run in range(n_runs):
    ...
    # Remove random_state as KNeighborsClassifier does not support it
    loaded_params.pop('random_state', None) 
    
    # Create and train the LightGBM model using the loaded hyperparameters
    KNN_model = KNeighborsClassifier(**loaded_params)
    KNN_model.fit(SX_train, ytrain_np)
    
    # Collect metrics for training, validation, and test sets
    train_metrics = calculate_metrics(SX_train, ytrain_np, KNN_model)
    test_metrics = calculate_metrics(SX_test, ytest_np, KNN_model)
    val_metrics = calculate_metrics(SXval, yval_np, KNN_model)
...
joblib.dump(best_model, MODEL_FOLDER + SYSTEM_SEP + f'best_KNN_noki_model_run_{best_run}.joblib')
